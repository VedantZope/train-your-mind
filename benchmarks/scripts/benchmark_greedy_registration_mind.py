"""Compute keypoint distances after GreedyRegistration using MIND descriptors."""
...
from mind.utils.io import load_keypoints_csv, mind_descriptor_image_convex
...
def run_greedy_registration_mind(
    masked: bool,
    dataset: str = "lungct",
    ...
    out_dir = root / "benchmarks" / dataset / (
        "greedy_registration_mind_masked"
        if masked
        else "greedy_registration_mind_nomask"
    )
    out_dir.mkdir(parents=True, exist_ok=True)

    pairs = build_pairs(root, dataset=dataset, dataset_root=dataset_root)
    if not pairs:
    ...
    with out_csv.open("w", newline="") as f:
        ...
        for sample in pairs:
            ...
            fixed_mind = mind_descriptor_image_convex(fixed_img)
            moving_mind = mind_descriptor_image_convex(moving_img)

            if masked:
                fixed_mind = apply_mask_to_image(fixed_mind, fixed_mask)
                moving_mind = apply_mask_to_image(moving_mind, moving_mask)

            fixed_batch = BatchedImages([fixed_mind])
            moving_batch = BatchedImages([moving_mind])
            ...
            writer.writerow(
                [case_id, fixed_path.name, moving_path.name, initial_dist, final_dist]
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Greedy registration benchmark (MIND descriptors)"
    )
